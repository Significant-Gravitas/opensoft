...
async def load_content_for_pass_tests_strict(module, pick_item):
    ...
    models = print_file_content(f"pages/{module}/models.py")
    implementation = print_file_content(
        f"pages/{module}/implementations/{module}_{IMPLEMENTATION_NUMBER}.py"
    )
    fixtures = print_file_content(f"pages/{module}/conftest.py")
    pytest_failure = RunnerPytest1().get_pytest_failure(
        n=int(pick_item), path=f"pages/{module}"
    )
...
async def load_content_for_pass_tests(module, pick_item):
    ...
    db_hint = "To use the db:\n with Session(engine) as session:\n    pass"
    models = print_file_content(f"pages/{module}/models.py")
    implementation = print_file_content(f"pages/{module}/{backend}/endpoint.py")
    fixtures = ""
...
async def load_content_for_add_tests(module, only_gherkin=True):
    product_requirements = print_file_content(f"pages/{module}/product_requirements.txt")
    user_stories = print_file_content(f"pages/{module}/user_stories.txt")
    models = print_file_content(f"pages/{module}/models.py")
    fixtures = print_file_content(f"pages/{module}/conftest.py")
    tests = print_file_content(f"pages/{module}/tests/test_{module}.py")
    if only_gherkin:
...
async def load_content_for_remove_tests(module):
    ...
    models = print_file_content(f"pages/{module}/models.py")
    fixtures = print_file_content(f"pages/{module}/conftest.py")
    test_positive = print_file_content(f"pages/{module}/tests/test_{module}.py")
    instructions = """
INSTRUCTIONS:
...
async def load_content_for_fix_frontend(module):
    app = print_file_content(f"pages/App.js")

    module_frontends = print_file_content(f"pages/{module}/UserFeedbackV2.js")
