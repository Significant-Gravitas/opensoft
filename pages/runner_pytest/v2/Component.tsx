...
const PytestFailures = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      `http://127.0.0.1:8000/v1/b1/pytest_failures?n=${testNumber}&path=${testPath}`,
      {
        method: 'GET',
        headers: {
    ...
    if (response.ok) {
      ...
      if (data) {
        ...
        setMessage(
          'No failure found for the given test number in the specified path.',
        );
        setTestResult('');
      }
    } else {
  ...
  return (
    ...
    >
      ...
      <form
        ...
        style={{ width: '300px', textAlign: 'center', marginBottom: '20px' }}
      >
        <label>
          Test File Path:
          <input
      ...
      {testResult && (
        <div
          style={{ textAlign: 'center', width: '100%', overflowX: 'scroll' }}
        >
