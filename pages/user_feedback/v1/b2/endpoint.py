...
async def feedback(feedback_data: FeedbackCreate):
    with Session(engine) as session:
        ...
        # Ensure that feedback_instance.id is set, else raise an exception
        if feedback_instance.id is None:
            raise ValueError("Failed to retrieve feedback ID after committing to the database.")
