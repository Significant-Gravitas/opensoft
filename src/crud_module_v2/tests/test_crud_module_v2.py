...
async def test_module_lifecycle(client):
    ...
    response = await client.get("/v2/modules/")
    ...
    response = await client.get(f"/v2/modules/{module_name}/")
    assert response.status_code == 404  # Assuming 404 for not found

    # 2. Create the module
    module_data = {"name": module_name}
    response = await client.post("/v2/modules/", json=module_data)
    ...
    # 5. Delete the module
    response = await client.delete(f"/v2/modules/{module_name}/")
