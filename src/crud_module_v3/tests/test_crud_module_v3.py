...
async def test_module_lifecycle(client):
    ...
    response = await client.get("/v3/modules/")
    ...
    response = await client.get(f"/v3/modules/{module_name}/")
    assert response.status_code == 404  # Assuming 404 for not found

    # 2. Create the module
    module_data = {"name": module_name}
    response = await client.post("/v3/modules/", json=module_data)
    ...
    # 5. Delete the module
    response = await client.delete(f"/v3/modules/{module_name}/")
